...
#ifndef INC_912LIST_H
#define INC_912LIST_H
...
protected:
   ...
   ListNodePosi<T> merge( ListNodePosi<T> p, Rank r, List<T>& L, ListNodePosi<T> q, Rank s); //归并
   void mergeSort( ListNodePosi<T>& p, Rank r); //对从p开始连续的n个节点归并排序
   void selectionSort( ListNodePosi<T> p, Rank r); //对从p开始连续的n个节点选择排序
   void insertionSort( ListNodePosi<T> p, Rank r); //对从p开始连续的n个节点插入排序
   void radixSort( ListNodePosi<T> p, Rank r); //对从p开始连续的n个节点基数排序
...
// 只读访问接口
   ...
   ListNodePosi<T> find( T const& e, Rank n, ListNodePosi<T> p ) const; //无序区间查找  从后往前
...
// 可写访问接口
   ...
   Rank dedup(); //无序去重
   Rank uniquify(); //有序去重
   void reverseA(); //前后倒置（习题）
   void reverseB(); //前后倒置（习题）
// 遍历
