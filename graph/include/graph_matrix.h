...
#ifndef INC_912GRAPH_GRAPH_MATRIX_H
#define INC_912GRAPH_GRAPH_MATRIX_H

#include "../../vector/include/vector.h" //引入向量
#include "graph.h" //引入图ADT

template <typename Tv> struct Vertex { //顶点对象（为简化起见，并未严格封装）
   Tv data; int inDegree, outDegree; VStatus status; //数据、出入度数、状态
   Rank dTime, fTime; //时间标签  dTime被发现  fTime被访问完毕
...
// 顶点的动态操作
    virtual Rank insert(Tv const &vertex) { //插入顶点，返回编号
        for (Rank u = 0; u < n; u++) E[u].insert(NULL);
        n++; //各顶点预留一条潜在的关联边
...
#endif //INC_912GRAPH_GRAPH_MATRIX_H
